...
class MultiLevelPT extends PT {
  ...
  override List<Double> fixedSizeOptimalPartition(MonotoneCubicSpline cumulativeLambdaEstimate, int size) {
    ...
    val n1_computeAware = findN1OutOf2(lambda1, lambda2, size, configs.b, true) 
    val n1_std = findN1OutOf2(lambda1, lambda2, size, configs.b, false)
    val n1 = (n1_computeAware + n1_std) / 2
    val n2 = size - n1 - 1
  ...
  static def int findN1OutOf2(double lambda1, double lambda2, int n, double discount, boolean computeAware) {
    ...
    for (k : 1 .. (n-1)) {
      ...
      val cur = 
        ...
        ) * (if (computeAware) (n1 / discount + n2) else 1.0)
      if (cur < min) {
        min = cur
        argmin = k
      }
